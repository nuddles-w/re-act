#!/usr/bin/env python3
"""
腾讯股票交易策略回测
对比两种策略：固定 5% vs 分层卖出
"""

import yfinance as yf
import pandas as pd
from datetime import datetime, timedelta

INITIAL_CAPITAL = 1_000_000
TRIGGER_GAIN = 0.30
SELL_INTERVAL = 0.10
BUY_BACK_DROP = 0.10
BUY_RATIO = 0.05

def get_sell_ratio_tiered(gain_pct):
    """分层卖出比例"""
    if gain_pct < 0.50:
        return 0.03
    elif gain_pct < 1.00:
        return 0.05
    else:
        return 0.08

def backtest_tencent(strategy='fixed'):
    """
    strategy: 'fixed' (固定5%) 或 'tiered' (分层)
    """
    print(f"\n{'='*70}")
    print(f"腾讯控股 (TCEHY) - {'分层卖出策略' if strategy == 'tiered' else '固定5%策略'}")
    print(f"{'='*70}")

    # 获取数据
    end_date = datetime.now()
    start_date = end_date - timedelta(days=3*365)
    data = yf.download('TCEHY', start=start_date, end=end_date, progress=False)

    if data.empty:
        print("❌ 无法获取数据")
        return None

    print(f"✓ 获取到 {len(data)} 天的数据 ({data.index[0].date()} ~ {data.index[-1].date()})")

    # 初始化
    initial_price = float(data['Close'].iloc[0])
    cash = 0
    shares = INITIAL_CAPITAL / initial_price
    cost_basis = initial_price

    trades = []
    sell_levels = []

    print(f"  初始价格: ${initial_price:.2f}")
    print(f"  初始股数: {shares:.2f}")
    print(f"  触发价格: ${initial_price * (1 + TRIGGER_GAIN):.2f} (+30%)\n")

    # 回测循环
    for date, row in data.iterrows():
        price = float(row['Close'])
        gain_from_cost = (price - cost_basis) / cost_basis

        # 卖出逻辑
        if gain_from_cost >= TRIGGER_GAIN:
            if not sell_levels or price > max(sell_levels) * (1 + SELL_INTERVAL):
                if strategy == 'tiered':
                    sell_ratio = get_sell_ratio_tiered(gain_from_cost)
                else:
                    sell_ratio = 0.05

                sell_shares = shares * sell_ratio
                sell_amount = sell_shares * price
                cash += sell_amount
                shares -= sell_shares
                sell_levels.append(price)

                trades.append({
                    'date': date.date(),
                    'action': 'SELL',
                    'price': price,
                    'shares': sell_shares,
                    'amount': sell_amount,
                    'gain_pct': gain_from_cost * 100,
                    'sell_ratio': sell_ratio * 100
                })
                print(f"📉 {date.date()} SELL: ${price:.2f} | 卖出 {sell_shares:.2f} 股 (${sell_amount:,.0f}) | 涨幅 +{gain_from_cost*100:.1f}% | 比例 {sell_ratio*100:.0f}%")

        # 买入逻辑
        if sell_levels:
            for sell_price in sell_levels[:]:
                buy_back_price = sell_price * (1 - BUY_BACK_DROP)
                if price <= buy_back_price and cash > 0:
                    total_value = cash + shares * price
                    buy_amount = min(total_value * BUY_RATIO, cash)
                    buy_shares = buy_amount / price

                    if buy_shares > 0:
                        cash -= buy_amount
                        shares += buy_shares
                        sell_levels.remove(sell_price)

                        trades.append({
                            'date': date.date(),
                            'action': 'BUY',
                            'price': price,
                            'shares': buy_shares,
                            'amount': buy_amount,
                            'gain_pct': gain_from_cost * 100,
                            'sell_ratio': None
                        })
                        print(f"📈 {date.date()} BUY:  ${price:.2f} | 买入 {buy_shares:.2f} 股 (${buy_amount:,.0f}) | 回调至 ${sell_price:.2f} 以下")

    # 最终结果
    final_price = float(data['Close'].iloc[-1])
    final_value = cash + shares * final_price
    total_return = (final_value - INITIAL_CAPITAL) / INITIAL_CAPITAL
    annualized_return = (final_value / INITIAL_CAPITAL) ** (1/3) - 1

    # 买入持有
    buy_hold_shares = INITIAL_CAPITAL / initial_price
    buy_hold_value = buy_hold_shares * final_price
    buy_hold_return = (buy_hold_value - INITIAL_CAPITAL) / INITIAL_CAPITAL

    print(f"\n{'='*70}")
    print(f"📊 策略表现：")
    print(f"  最终价值:     ${final_value:,.0f}")
    print(f"  总收益率:     {total_return*100:.2f}%")
    print(f"  年化收益率:   {annualized_return*100:.2f}%")
    print(f"\n💰 持仓明细：")
    print(f"  现金:         ${cash:,.0f} ({cash/final_value*100:.1f}%)")
    print(f"  股票市值:     ${shares * final_price:,.0f} ({shares*final_price/final_value*100:.1f}%)")
    print(f"  当前股价:     ${final_price:.2f}")
    print(f"\n📈 对比买入持有：")
    print(f"  买入持有价值: ${buy_hold_value:,.0f}")
    print(f"  买入持有收益: {buy_hold_return*100:.2f}%")
    print(f"  策略差异:     {(total_return - buy_hold_return)*100:+.2f}%")
    print(f"\n📝 交易统计：")
    print(f"  总交易:       {len(trades)} 次")
    print(f"  卖出:         {sum(1 for t in trades if t['action'] == 'SELL')} 次")
    print(f"  买入:         {sum(1 for t in trades if t['action'] == 'BUY')} 次")

    # 保存交易记录
    if trades:
        df_trades = pd.DataFrame(trades)
        filename = f'/Users/bytedance/re-act/trades_TCEHY_{strategy}.csv'
        df_trades.to_csv(filename, index=False)
        print(f"  ✓ 交易记录已保存至 trades_TCEHY_{strategy}.csv")

    return {
        'strategy': strategy,
        'final_value': final_value,
        'total_return': total_return,
        'annualized_return': annualized_return,
        'buy_hold_return': buy_hold_return,
        'strategy_diff': total_return - buy_hold_return,
        'cash': cash,
        'shares': shares,
        'num_trades': len(trades)
    }

# 执行两种策略
print("\n" + "="*70)
print("腾讯控股 (TCEHY) 策略对比")
print("="*70)

result_fixed = backtest_tencent('fixed')
result_tiered = backtest_tencent('tiered')

# 对比
if result_fixed and result_tiered:
    print(f"\n\n{'='*70}")
    print(f"策略对比总结")
    print(f"{'='*70}\n")
    print(f"{'策略':<15} {'总收益率':<12} {'年化收益':<12} {'vs买入持有':<12} {'交易次数':<10}")
    print(f"{'-'*70}")
    print(f"{'固定5%':<13} {result_fixed['total_return']*100:>10.2f}%  {result_fixed['annualized_return']*100:>10.2f}%  {result_fixed['strategy_diff']*100:>10.2f}%  {result_fixed['num_trades']:>8}次")
    print(f"{'分层卖出':<13} {result_tiered['total_return']*100:>10.2f}%  {result_tiered['annualized_return']*100:>10.2f}%  {result_tiered['strategy_diff']*100:>10.2f}%  {result_tiered['num_trades']:>8}次")
    print(f"{'买入持有':<13} {result_fixed['buy_hold_return']*100:>10.2f}%  {(result_fixed['buy_hold_return']/3)*100:>10.2f}%  {'0.00%':>12}  {'0':>8}次")

    diff = result_tiered['total_return'] - result_fixed['total_return']
    print(f"\n分层策略 vs 固定5%: {diff*100:+.2f}%")
