...
多股票交易策略回测 - 分层卖出版本
策略优化：
1. 30-50% 涨幅: 每涨 10% 卖出 3%
2. 50-100% 涨幅: 每涨 10% 卖出 5%
3. 100%+ 涨幅: 每涨 10% 卖出 8%
4. 跌回卖出价位以下 10% 时买入 5%
"""
...
# 分层卖出比例
def get_sell_ratio(gain_pct):
    """根据涨幅返回卖出比例"""
    if gain_pct < 0.50:      # 30-50%
        return 0.03
    elif gain_pct < 1.00:    # 50-100%
        return 0.05
    else:                     # 100%+
        return 0.08
...
def backtest_stock(ticker, name):
    ...
    for date, row in data.iterrows():
        ...
        if gain_from_cost >= TRIGGER_GAIN:
            if not sell_levels or price > max(sell_levels) * (1 + SELL_INTERVAL):
                sell_ratio = get_sell_ratio(gain_from_cost)
    ...
    if trades:
        ...
        df_trades.to_csv(f'/Users/bytedance/re-act/trades_{ticker}_tiered.csv', index=False)
        print(f"  ✓ 交易记录已保存至 trades_{ticker}_tiered.csv")
...
# 保存汇总
df_summary.to_csv('/Users/bytedance/re-act/backtest_summary_tiered.csv', index=False)
print(f"\n✓ 汇总结果已保存至 backtest_summary_tiered.csv")

# 对比原策略
print(f"\n\n{'='*70}")
print(f"策略对比：分层卖出 vs 固定 5% 卖出")
print(f"{'='*70}\n")
print("分层卖出策略：")
print("  30-50% 涨幅: 每涨 10% 卖出 3%")
print("  50-100% 涨幅: 每涨 10% 卖出 5%")
print("  100%+ 涨幅: 每涨 10% 卖出 8%")
print("\n原策略：")
print("  所有涨幅: 每涨 10% 卖出 5%")
