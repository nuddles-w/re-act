...
腾讯网格交易策略回测
策略：
1. 低于 500 港币，每下跌 10% 买入 1 手（100 股）
2. 涨到 580 港币开始，每上涨 10 港币卖出 1 手（100 股）
"""
...
print(f"  买入触发: < HK${BUY_TRIGGER} (每跌 10% 买 100 股)")
print(f"  卖出触发: > HK${SELL_TRIGGER} (每涨 HK$10 卖 100 股)")
print(f"\n开始回测...\n")
...
for date, row in tencent.iterrows():
    ...
    if price_hkd < BUY_TRIGGER:
        ...
        if level_index not in buy_levels_triggered and price_hkd <= buy_price_level:
            ...
            if cash >= cost:
                ...
                print(f"📈 {date.date()} BUY:  HK${price_hkd:.2f} | 买入 100 股 (¥{cost:,.0f}) | 档位 -{level_index*10}% | 余额 ¥{cash:,.0f}")

    # 卖出逻辑：高于 580，每涨 10 HKD 卖出 1 手
    if price_hkd > SELL_TRIGGER and shares >= SHARES_PER_TRADE:
        ...
        if level_index not in sell_levels_triggered and price_hkd >= sell_price_level:
            ...
            print(f"📉 {date.date()} SELL: HK${price_hkd:.2f} | 卖出 100 股 (¥{revenue:,.0f}) | 档位 +{level_index*10} HKD | 余额 ¥{cash:,.0f}")
...
if trades:
    ...
    df_trades.to_csv('/Users/bytedance/re-act/trades_TCEHY_grid.csv', index=False)
    print(f"\n✓ 交易记录已保存至 trades_TCEHY_grid.csv")
